...
def train(model,sess,saver,epochs,batch_size,data_train,data_test,id2word,id2tag):    
    ...
    for epoch in range(epochs):
        ...
        if epoch%3==0:
            ...
            Kreuzung = [i for i in entityres if i in entityall]
            if len(Kreuzung)!=0:
                precision = float(len(Kreuzung))/len(entityres)
                recall = float(len(Kreuzung))/len(entityall)
...
def test_input(model,sess,word2id,id2tag,batch_size):
    while True:
        text = input("Enter your input: ").encode('utf-8').decode('utf-8');
